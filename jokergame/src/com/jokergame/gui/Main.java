	...
	private void initialize() {
		...
		mainPanel.addComponentListener(new ComponentAdapter() {
			...
			public void componentShown(ComponentEvent e) {
				...
				msg.append(" ");
				msg.append(memberDTO.getNickname() + "´Ô ¹Ý°©½À´Ï´Ù");
				lblMain.setText(msg.toString());
				
				//tblMain ÃÊ±âÈ­
				//ÇöÀç ´ë±âÁßÀÎ ¹æ ¸ñ·Ï °¡Á®¿Í¼­ Ãâ·ÂÇÏ±â
				String[][] rooms;
				try {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					if(memberDAO.login(id, pw)) {
						//id¿Í pwºñ±³ °á°ú°¡ °°À¸¸é ¸ÞÀÎ ÆäÀÌÁö·Î
						loginPanel.setVisible(false); //·Î±×ÀÎÆäÀÌÁö ºñÈ°¼ºÈ­
						memberDTO = memberDAO.getMember(id); //¸â¹ö Á¤º¸ ÀúÀå
						memberDAO.setState(id, 1); //¸â¹ö »óÅÂ¸¦ 1(·Î±×ÀÎ)À¸·Î º¯°æ
						
						mainPanel.setVisible(true); //¸ÞÀÎÆäÀÌÁö È°¼ºÈ­
					}
				} catch (SQLException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
		...
		btnJoin.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (txtJoinPwChk.getText().equals(txtJoinPw.getText())) {
					MemberDTO mDTO = new MemberDTO(); // MemberDTO °´Ã¼ »ý¼º ¹× ÃÊ±âÈ­
